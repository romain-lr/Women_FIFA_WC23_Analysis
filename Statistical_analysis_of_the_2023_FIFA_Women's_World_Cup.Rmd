...
#Testing the model with all df6 variables without interaction
df_model_6 <- df_model_6 %>% mutate(distance_to_center = case_when(location.y >40 ~ location.y-40,
                                                                   location.y<=40 ~ -(location.y-40)))
glm.model_6 <- glm(shot.outcome.name ~ ., data = df_model_6, family = binomial(link = "logit"))
summary(glm.model_6)
...
This model proves that the distance to the central axis of the pitch is very significant, with a p-value<1e-4. However, findings suggest that y-positions are irrelevant for the goalkeeper, as only x-positions are significant in the above model.

Our best model is composed of 6 variables : 
The technique of shot, the type of shot, the body part used, the distance to the center and the positions in x for both the player and the opposing goalkeeper.\
