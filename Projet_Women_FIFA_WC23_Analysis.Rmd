...
```{r fig11 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig11}StatsBomb xG ratio on our xG ratio based on bestmod for players",fig.height=4.5,fig.align='center'}
...
We can see that for the vast majority of shots, our models are similar! Indeed, both models yield similar results for shots with low xG values, specifically those with a scoring probability of approximately 0.25 or less. However, our model appears to lack the training base and necessary parameters to account for certain probabilities that StatsBomb assigns a value exceeding 25%. This discrepancy is represented by the orange and red points in the upper right of the graph. Nonetheless, we observe some similarities around an expected goal value of 0.75. Upon investigation, these points correspond to penalties, which StatsBomb consistently classifies with an xG of 0.785, and our model similarly places within this range. Our model does not assign the same value to all penalties because it adjusts the xG based on one of the included variables, the goalkeeper's position, modifying the xG if the goalkeeper is slightly to the right, for example. Additionally, since the model considers the body part used for the shot, our dataset may be slightly biased towards one foot, either the right or the left.

These assertions and hypotheses can be validated or refuted using visualizations such as the one in \ref{fig:fig14}, which displays the characteristics of the shots. In this figure, we present the types of shots for the 100 shots with the greatest differences between the Expected Goals predictions of the two models:
