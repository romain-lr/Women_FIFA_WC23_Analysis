...
```{r, echo = F, eval = T, warning=FALSE, fig1 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig1}Visualization of the shots of Spain-England on the pitch",fig.height=3.5, fig.width = 3.5}
...
```{r fig2 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig2}Diagram of the number of goals and shots in all matches for each team",fig.height=4.5}
#Let's make a graph
...
```{r fig3 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig3}Diagram of the percentage of shots leading to a goal",fig.height=4,fig.align='center'}
...
```{r fig4 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig4}Diagram of the number of shots and goals for each French and percentage",fig.height=2,fig.align='center'}
...
```{r fig5 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig5}Diagram of the number of shots and goals for each type of shot",fig.height=2,fig.align='center'}
...
```{r fig8 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig8}Goal ratio on the number of totals shots for players",fig.height=4.5,fig.align='center'}
...
```{r fig9 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig9}Goal ratio on the Expected Goal ratio (StatsBomb model) for players",fig.height=4,fig.align='center'}
...
```{r fig10 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig10}Goal ratio on the number of total shots for teams",fig.width=6,fig.align='center'}
...
```{r fig11 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig11}Goal ratio on the Expected Goal ratio (StatsBomb model) for teams",fig.width=8,fig.align='center'}
...
```{r fig12 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig12}StatsBomb xG ratio on our xG ratio based on bestmod for players",fig.height=4.5,fig.align='center'}
...
```{r fig13 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig13}StatsBomb xG  on our xG  based on bestmod for every shots in the World Cup",fig.height=4.5,fig.align='center'}
...
```{r fig14 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig14}StatsBomb xG  on our xG  based on bestmod and the type of shots for the 100 shots with the higher xG difference",fig.width=8,fig.align='center'}
...
```{r fig15 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig15}True Pass ratio on the Total Number of Pass for every player with 5 pass or more during the WC",fig.width=10,fig.align='center'}
...
```{r fig16 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig16}True Pass ratio on the Total Number of Pass for every teams during the WC",fig.width=10,fig.align='center'}
...
```{r fig17 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig17}True Pass ratio on our model xP ratio for the 25% hardest pass for every country",fig.width=10,fig.align='center'}
...
In figure \ref{fig:fig2} shows a visualization of these results.\, we observe several interesting points in this graph. Spain, having made more than 5,000 passes, has a significantly lower proportion of difficult passes compared to other teams. On average, Spain's passes were simpler, even considering only the hardest 25% of passes according to our xP model. This suggests that their style of play relied more on possession, involving ball circulation and fewer challenging passes, such as long crosses or through balls during counterattacks, likely due to higher ball possession. Furthermore, consistent with our analysis of Spain, the strongest teams are clustered together, along with some less strong teams, around an xP slightly below 70%. This indicates that their most difficult passes had, on average, a 70% chance of success, which is quite substantial.
...
Figure \ref{fig:fig18} presents a visualization of these results.\
...
```{r fig18 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig18}True Pass ratio on our model xP ratio for the 25% easiest pass for every country during the WC",fig.width=10,fig.align='center'}
p9 <- ggplot(df2ptail_easy, aes(xP_ratio, pass_completion_ratio, size = total_pass, label=team, color = groupv2)) + geom_text(aes(fontface=3)) + geom_abline(intercept = 0, slope = 1, linetype = "dashed") + scale_color_viridis(discrete=TRUE)

p9 <- p9 + geom_point(data = df2ptail_easy[df2ptail_easy$team == "Mean Country", ], aes(xP_ratio, pass_completion_ratio,  size = total_pass), shape = 4, color = "red4", stroke = 1)  + labs(x = "Our Model's Prediction of xP", y = "True Pass Ratio", fill = "World Cup's Stages", color = "World Cup's Stages")
