---
#title: "Statistical analysis of the 2023 FIFA Women's World Cup"
...
```{r, echo = F, eval = TRUE, out.width = '450px', out.height = '200px', fig.align='center'}
knitr::include_graphics('./Image_pied_de_page.PNG')
```
...
# Create a dataframe for the scale color
...
```{r, echo = F, eval = T, warning=FALSE, fig1 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig1}Visualization of the shots of Spain-England on the pitch",fig.height=3.5, fig.width = 3., fig.pos='H'}
...
Figure \ref{fig:fig2} shows a visualization of these results.\
...
```{r fig2 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig2}Diagram of the number of goals and shots in all matches for each team",fig.height=4.5, fig.pos='H'}
#Let's make a graph
...
Therefore, we calculated the percentage of shots leading to a goal for every team, and compared them in Figure \ref{fig:fig3}.
...
```{r fig3 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig3}Diagram of the percentage of shots leading to a goal",fig.height=4,fig.align='center', fig.pos='H'}
...
Figure \ref{fig:fig4} shows that there were a huge number of goals in two games : Panama-France and Australia-France. The final score of the first match was 3-6 for France, while the latter ended on a draw. However, teams went to penalties, and they scored a total of 13, making this observation flawed.
...
```{r fig4 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig4}Diagram of the number of shots and goals for each French and percentage",fig.height=2,fig.align='center', fig.pos='H'}
...
We first look at the different types of shots in Figure \ref{fig:fig5}. Four different types of shots were differentiated in the dataset : "Open Play", "Penalty", "Free Kick" as well as "Corner." A shot was deemed as being "Open Play" if it was taken during regular actions of the game. The "Corner" label only applies to a single shot made by Ireland's Katie McCabe that went in. This is something to keep in mind, as it is sure to skew our future models. Other labels are self-explanatory.\
...
```{r fig5 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig5}Diagram of the number of shots and goals for each type of shot",fig.height=2,fig.align='center', fig.pos='H'}
...
Figure \ref{fig:fig6} shows that the dataset contains seven types of shots, although only three are consistently being used, that is : "Normal", "Half Volley" and "Volley". Naturally, the "Normal" shot was the most popular, and hence yielded the mos t goals. A "Lob" was very infrequently done, but could prove effective in the right situation : it seems a good proportion of these shots went in.
...
```{r fig6 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig6}Diagram of the number of shots and goals for each type of shot",fig.height=2,fig.pos="h",fig.align='center'}
...
Similarly, we visualize in Figure \ref{fig:fig7} the different body parts used in shooting.
...
```{r fig7 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig7}Diagram of the number of goals and shots according to body zone used",fig.height=2,fig.pos="h"}
...
Figure \ref{fig:fig8} shows a visualization of the goal ratio of players that performed at least 1 shot during the World Cup.
...
```{r fig8 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig8}Goal ratio on the number of totals shots for players",fig.height=3,fig.align='center', fig.pos='H'}
...
This his what the Figure \ref{fig:fig9} is showing true goal ratio on the StatsBomb xG ratio for every player which attempted at least 1 shots during the World Cup.
...
```{r fig9 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig9}Goal ratio on the Expected Goal ratio (StatsBomb model) for players",fig.height=4,fig.align='center', fig.pos='H'}
...
Figure \ref{fig:fig10} visualizes the goal ratio relative to the total shots taken by teams that made at least one attempt during the World Cup.
...
```{r fig10 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig10}Goal ratio on the number of total shots for teams",fig.height=4,fig.width=8,align='center', fig.pos='H'}
...
Figure \ref{fig:fig11} shows the true goal ratio relative to the StatsBomb xG ratio for each team during the World Cup.
...
```{r fig11 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig11}Goal ratio on the Expected Goal ratio (StatsBomb model) for teams",fig.width=8,fig.align='center', fig.pos='H'}
...
The technique of shot, the type of shot, the body part used, and the positions in x for both the player and the opposing goalkeeper.\
...
Figure \ref{fig:fig12} presents a visualization of these results.\
...
# Calculate the predicted probability of success for each pass (xG)
...
```{r fig12 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig12}StatsBomb xG ratio on our xG ratio based on bestmod for players",fig.height=4.5,fig.align='center', fig.pos='H'}
...
This is depicted in Figure \ref{fig:fig13} below:\

```{r fig13 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig13}StatsBomb xG  on our xG  based on bestmod for every shots in the World Cup",fig.height=4,fig.align='center', fig.pos='H'}
pp <-ggplot(Shots_exp_predi, aes(xG_predi, shot.statsbomb_xg)) + geom_point(shape=21) + geom_abline(intercept = 0, slope = 1, linetype = "dashed") + labs(x = "Our Model's Prediction of the xG's ratio", y = "StatsBomb's Model xG's Ratio", fill = "Differences of xG", color = "Differences of xG")

ggplotly(pp)
pp
```

The results indicate that our models are comparable for the majority of shots. Nonetheless, we do observe some outliers, which may be elucidated by including additional factors such as the type of shot, whether it is a free kick, an open play shot, a penalty, a corner, and so forth. This is illustrated in Figure \ref{fig:fig14} below:\
...
```{r fig14 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig14}StatsBomb xG  on our xG  based on bestmod and the type of shots for the 100 shots with the higher xG difference",fig.width=8,fig.align='center', fig.pos='H'}
...
```{r fig15 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig15}True Pass ratio on the Total Number of Pass for every player with 5 pass or more during the WC",fig.width=10,fig.align='center', fig.pos='H'}
...
The graph on \ref{fig:fig15} depicts the ratio of successful passes to the total number of passes. Once again, a noticeable disparity is evident: players from countries that advanced to the quarterfinals occupy the uppermost positions on the graph, boasting a successful pass ratio exceeding 60%, whereas teams eliminated in the round of 16 and group stages display notably lower pass ratios. It is worth mentioning that even within these less successful teams, a considerable number of players exhibit pass ratios comparable to those in the top-performing teams. This suggests potential variations within these countries, likely indicating a greater degree of diversity in player performance.\
...
```{r fig16 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig16}True Pass ratio on the Total Number of Pass for every teams during the WC",fig.width=10,fig.align='center', fig.pos='H'}
...
```{r,include=F}
#The location.y parameter was not considered relevant enough for the model. Adding the distance to center, as the location.y attribute should have a symmetrical impact along the half line (the ones passing through the goalposts) from a strategical standpoint. 
...
### Comparaison xP to True Pass Ratio - Hardest Passes 
...
```{r,include=F}
pays2 <- melt(df2p_hard$team) %>% distinct() %>% as.list() %>% unlist() %>% sort() %>% as.list() #select all teams
#view(pays2)

df2p_pays_hard <- df2p_hard[df2p_hard$team %in% pays2,]
#view(df2p_pays)

for (i in 0:length(pays2))
  df2p_pays_hard[nrow(df2p_pays_hard) + 1, ] <- list(paste("",pays2[i]), paste("", pays2[i]),
                                        sum(df2p_pays_hard$total_pass[df2p_pays_hard$team == pays2[i]]),
                                        sum(df2p_pays_hard$pass_completion_ratio[df2p_pays_hard$team == pays2[i]]),
                                        sum(df2p_pays_hard$total_pass_succeed[df2p_pays_hard$team == pays2[i]]),
                                        sum(df2p_pays_hard$xP[df2p_pays_hard$team == pays2[i]]),
                                        0)
  
df2p_pays_hard <- subset(df2p_pays_hard, select = -c(player))

#view(df2p_pays)
n <- length(pays2)

df2ptail_hard <- df2p_pays_hard[(nrow(df2p_pays_hard)-(n - 1)):nrow(df2p_pays_hard),]  %>% mutate(team = gsub(" ","",team),
                                                                    pass_completion_ratio = total_pass_succeed/total_pass,
                                                                    total_pass = total_pass,
                                                                    total_pass_succeed = total_pass_succeed,
                                                                    xP = xP,
...
df2ptail_hard$groupv2 <- ifelse(df2ptail_hard$team %in% pays_gs, "8-GS",
                              ifelse(df2ptail_hard$team %in% pays_r16, "7-R16",
                                     ifelse(df2ptail_hard$team %in% pays_quarterfinals, "6-Quarterfinals",
                                            ifelse(df2ptail_hard$team == "Australia", "4th",
                                                   ifelse(df2ptail_hard$team == "Sweden", "3rd",
                                                          ifelse(df2ptail_hard$team == "England", "2nd",
                                                                 ifelse(df2ptail_hard$team=="Spain","1st","NA")))))))


mean_pass_nb_pays = mean(df2ptail_hard$total_pass)
mean_pass_success_pays = mean(df2ptail_hard$total_pass_succeed)
mean_pass_completion_ratio_pays = mean_pass_success_pays/mean_pass_nb_pays
mean_pass_xP_pays = mean(df2ptail_hard$xP)
mean_pass_xP_ratio_pays = mean_pass_xP_pays/mean_pass_nb_pays

df2ptail_hard[nrow(df2ptail_hard) + 1,] <- list("Mean Country", mean_pass_nb_pays, mean_pass_completion_ratio_pays, mean_pass_success_pays, mean_pass_xP_pays, mean_pass_xP_ratio_pays, "Mean Country")


#view(df2ptail)
```

```{r fig17 ,echo=F,eval=TRUE,fig.cap="\\label{fig:fig17}True Pass ratio on our model xP ratio for the 25% hardest pass for every country",fig.width=8,fig.pos='H'}
p8 <- ggplot(df2ptail_hard, aes(xP_ratio, pass_completion_ratio, size = total_pass, label=team, color = groupv2)) + geom_text(aes(fontface=3)) + geom_abline(intercept = 0, slope = 1, linetype = "dashed")+ scale_color_viridis(discrete=TRUE)

p8 <- p8 + geom_point(data = df2ptail_hard[df2ptail_hard$team == "Mean Country", ], aes(xP_ratio, pass_completion_ratio,  size = total_pass), shape = 4, color = "red4", stroke = 1) + labs(x = "Our Model's Prediction of xP", y = "True Pass Ratio", fill = "World Cup's Stages", color = "World Cup's Stages")

#ggplotly(p8)
p8
```

We observe several interesting points in this graph. Spain, having made more than 5,000 passes, has a significantly lower proportion of difficult passes compared to other teams. On average, Spain's passes were simpler, even considering only the hardest 25% of passes according to our xP model. This suggests that their style of play relied more on possession, involving ball circulation and fewer challenging passes, such as long crosses or through balls during counterattacks, likely due to higher ball possession. Furthermore, consistent with our analysis of Spain, the strongest teams are clustered together, along with some less strong teams, around an xP slightly below 70%. This indicates that their most difficult passes had, on average, a 70% chance of success, which is quite substantial.
...
### Comparaison xP to True Pass Ratio - Easiest Passes
...
```{r,include=F}
pays2 <- melt(df2p_easy$team) %>% distinct() %>% as.list() %>% unlist() %>% sort() %>% as.list() #select all teams
#view(pays2)

df2p_pays_easy <- df2p_easy[df2p_easy$team %in% pays2,]
#view(df2p_pays_easy)

for (i in 0:length(pays2))
  df2p_pays_easy[nrow(df2p_pays_easy) + 1, ] <- list(paste("",pays2[i]), paste("", pays2[i]),
                                        sum(df2p_pays_easy$total_pass[df2p_pays_easy$team == pays2[i]]),
                                        sum(df2p_pays_easy$pass_completion_ratio[df2p_pays_easy$team == pays2[i]]),
                                        sum(df2p_pays_easy$total_pass_succeed[df2p_pays_easy$team == pays2[i]]),
                                        sum(df2p_pays_easy$xP[df2p_pays_easy$team == pays2[i]]),
                                        0)
  
df2p_pays_easy <- subset(df2p_pays_easy, select = -c(player))

#view(df2p_pays_easy)
n <- length(pays2)

df2ptail_easy <- df2p_pays_easy[(nrow(df2p_pays_easy)-(n - 1)):nrow(df2p_pays_easy),]  %>% mutate(team = gsub(" ","",team),
                                                                    pass_completion_ratio = total_pass_succeed/total_pass,
                                                                    total_pass = total_pass,
                                                                    total_pass_succeed = total_pass_succeed,
                                                                    xP = xP,
...
df2ptail_easy$groupv2 <- ifelse(df2ptail_easy$team %in% pays_gs, "8-GS",
                              ifelse(df2ptail_easy$team %in% pays_r16, "7-R16",
                                     ifelse(df2ptail_easy$team %in% pays_quarterfinals, "6-Quarterfinals",
                                            ifelse(df2ptail_easy$team == "Australia", "4th",
                                                   ifelse(df2ptail_easy$team == "Sweden", "3rd",
                                                          ifelse(df2ptail_easy$team == "England", "2nd",
                                                                 ifelse(df2ptail_easy$team=="Spain","1st","NA")))))))


mean_pass_nb_pays = mean(df2ptail_easy$total_pass)
mean_pass_success_pays = mean(df2ptail_easy$total_pass_succeed)
mean_pass_completion_ratio_pays = mean_pass_success_pays/mean_pass_nb_pays
mean_pass_xP_pays = mean(df2ptail_easy$xP)
mean_pass_xP_ratio_pays = mean_pass_xP_pays/mean_pass_nb_pays

df2ptail_easy[nrow(df2ptail_easy) + 1,] <- list("Mean Country", mean_pass_nb_pays, mean_pass_completion_ratio_pays, mean_pass_success_pays, mean_pass_xP_pays, mean_pass_xP_ratio_pays, "Mean Country")
